...
# New load calculation: absolute first + constant ship SWBM!
...
mu_Y_yield = 250.0
sigma_Y_yield = mu_Y_yield * cov_Y

print(f"Load Stats: mean={mu_L:.4e} N.m, std={sigma_L:.4e} N.m")
...
for t, W_pantau in zip(ages, W_pantau_vals):
    ...
    FoS = mu_Y_ult / mu_sigma_L
    beta = (mu_Y_ult - mu_sigma_L) / np.sqrt(sigma_Y_ult**2 + sigma_sigma_L**2)
    Pf = norm.cdf(-beta)
...
df_res_ult = pd.DataFrame(results_ult)
print("\n--- Ultimate Strength (412.14 MPa) Reliability ---")
print(df_res_ult.to_string(index=False))

# Yield strength reliability
results_yield = []
for t, W_pantau in zip(ages, W_pantau_vals):
    mu_sigma_L = (mu_L / W_pantau) * 1e-6
    sigma_sigma_L = (sigma_L / W_pantau) * 1e-6
    ...
    beta = (mu_Y_yield - mu_sigma_L) / np.sqrt(sigma_Y_yield**2 + sigma_sigma_L**2)
    Pf = norm.cdf(-beta)
...
df_res_yield = pd.DataFrame(results_yield)
print("\n--- Yield Strength (250 MPa) Reliability ---")
print(df_res_yield.to_string(index=False))
